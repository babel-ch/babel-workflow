...
동작 개요
  ...
  3. 턴을 블로킹하지 않도록 실제 작업(요약+전송)은 백그라운드 워커로 분리하고
     훅 본체는 곧바로 exit 0 한다.
  4. 워커: transcript에서 마지막 사용자 프롬프트 + 마지막 응답을 뽑아
     `claude -p`로 세션 제목 + 턴 요약(JSON)을 만든 뒤 Notion에 기록.
...
def extract_turn(transcript_path):
    """transcript JSONL에서 마지막 사용자 프롬프트와 마지막 응답 텍스트를 뽑는다."""
    last_prompt = ""
    last_assistant = ""
    try:
        ...
        return last_prompt, last_assistant
...
# ─────────────────────────────────────────────────────────────
def summarize(project, prompt, response, current_title):
